...
residuals_original_scale <- predictions_original_scale - HospitalStays$Length_of_Stay

# Calculate RMSE in the original scale
rmse_original <- sqrt(mean(residuals_original_scale^2))

# Calculate MAE in the original scale
mae_original <- mean(abs(predictions_original_scale - HospitalStays$Length_of_Stay))

# Print results
cat("RMSE original Scale for MLR Model 1:", rmse_original, "\n")
...
model_poly <- lm(log(Length_of_Stay) ~ Age + poly(Infection_Risk, 2) * Region + Avg_Daily_Census +  Routine_Chest_Xray_Ratio, data = HospitalStays)
...
# Print results
cat("RMSE original Scale for MLR Model 2:", rmse_original, "\n")
cat("MAE original Scale for MLR model 2:", mae_original, "\n")

```
### Remove the Outliers and verify the model performance

```{r}
str(HospitalStays)
HospitalStayswoOutliers <- HospitalStays[-c(47, 112), ]
str(HospitalStayswoOutliers)
```
Log transformation of Y with only the features selected by Lasso regression method 

```{r}
model3 <- lm(log(Length_of_Stay) ~ Age + Infection_Risk + Region + Routine_Chest_Xray_Ratio+ Avg_Daily_Census, data=HospitalStayswoOutliers)
...
# Calculate residuals in the original scale
residuals_original_scale <- predictions_original_scale - HospitalStayswoOutliers$Length_of_Stay

# Calculate RMSE in the original scale
rmse_original <- sqrt(mean(residuals_original_scale^2))

# Calculate MAE in the original scale
mae_original <- mean(abs(predictions_original_scale - HospitalStayswoOutliers$Length_of_Stay))
...
model_poly <- lm(log(Length_of_Stay) ~ Age + poly(Infection_Risk, 2) * Region + Avg_Daily_Census +  Routine_Chest_Xray_Ratio, data =HospitalStayswoOutliers)
